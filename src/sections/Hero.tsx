...
export const Hero: React.FC = () => {
  return (
    <section className="relative pt-20 sm:pt-28 pb-12 lg:pb-20 bg-[#F7F3EC] overflow-hidden">
      <Container size="xl">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start">
          {/* Left Column — Editorial Copy (positioned high to align naturally with upper/middle phone mockup) */}
          <motion.div
            initial={{ opacity: 0, y: 16 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.7, ease: [0.16, 1, 0.3, 1] }}
            className="lg:col-span-7 space-y-5 sm:space-y-6 text-left lg:pt-3"
          >
            {/* 1. Small Brand Badge */}
            ...
            {/* 2. Main Headline with intentional mobile line breaks */}
            <h1 className="font-serif text-4xl sm:text-6xl xl:text-7xl font-normal leading-[1.08] tracking-tight text-[#242326]">
              ...
              <span className="block sm:inline">moments,</span><br className="hidden sm:inline" />
              <span className="italic font-light text-[#6E2C3A] block sm:inline">made lasting.</span>
            </h1>
            ...
            {/* 4. Action Buttons (Primary CTA -> Secondary CTA) */}
            ...
            {/* 5. Micro Stats Row (Refined brand copy without unsupported 100% claim) */}
            <div className="pt-4 border-t border-[#D8D0C5]/60 flex flex-wrap sm:flex-nowrap items-center gap-4 sm:gap-6 text-xs text-[#625D59]">
              <div>
                <span className="font-serif text-lg font-medium text-[#242326]">Personalized</span>
                <p className="text-[11px]">Your Story</p>
              </div>
              <div className="h-6 w-px bg-[#D8D0C5] hidden sm:block" />
              <div>
          ...
          {/* Right Column — Standalone Mobile Phone Mockup (No decorative card frame behind) */}
          <motion.div
            initial={{ opacity: 0, scale: 0.97, y: 16 }}
            animate={{ opacity: 1, scale: 1, y: 0 }}
            transition={{ duration: 0.8, delay: 0.1, ease: [0.16, 1, 0.3, 1] }}
            className="lg:col-span-5 flex justify-center relative pt-2 lg:pt-0"
          >
            <MobilePhoneFrame />
          </motion.div>
        </div>
